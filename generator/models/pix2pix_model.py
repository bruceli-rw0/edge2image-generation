...
class Pix2Pix(BaseModel, nn.Module):
    ...
    and a '--gan_mode' vanilla GAN loss (the cross-entropy objective used in the 
    orignal GAN paper).
    ...
    def forward(self, label, image):
        ...
        self.real_A = label.to(self.device)
        self.real_B = image.to(self.device)
        self.fake_B = self.netG(self.real_A)  # compute fake images: G(A)
        return self.fake_B
